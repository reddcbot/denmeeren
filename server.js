...
client.on("message", msg => {
  ...
  const dmembed = new Discord.RichEmbed()
    ...
    .setDescription(
      " " +
        msg.author.tag +
        " Sunucunda Reklam Yapıyor r!uyar komutu ile kişiyi uyara bilir r!ban Komutu İle Kişiyi Banlayabilirsin "
    )
    .addField("Kullanıcının mesajı:", "**" + msg.content + "**");
...
client.on("guildMemberAdd", async member => {
  ...
  let embed = new Discord.RichEmbed()
    ...
    .setColor("GREEN")
    .setFooter("REDNO", client.user.avatarURL);
  ...
  client.on("guildMemberAdd", member => {
    let guild = member.guild;
    let role = guild.roles.find(role => role.name === "「❌」Kayıtsız Üye");
    const embed = new Discord.RichEmbed().setImage(
      ""
    );
    member.guild.channels.get("606138994950012959").sendEmbed(embed).then;
    member.addRole(role);
  });

  const wait = client.emojis.get("611190936617091072");
  const discordd = client.emojis.get("611238686088232960");
  const solkanat = client.emojis.get("610425615195176982");
  const sagkanat = client.emojis.get("6610425561193644038");
  const onaytik1 = client.emojis.get("610425561193644038");
  const kanat1 = client.emojis.get("611501048997085194");
  ...
  try {
    ...
    giriscikiskanali.send(`${kanat1} Sunucumuza hoşgeldin <@${member.user.id}> 
${kanat1} Kaydının yapılması için sesli odaya gelip ses vermen yeterli olucaktır.
${wait} Kayıt Sorumlusu olan yetkililer seninle ilgilenecektir.
:gem: Sunucuda keyifli vakit geçirmen dileğiyle ${solkanat}**${member.user.tag}** ${onaytik1}
`);
  } catch (e) {
...
client.on("guildMemberRemove", async member => {
  ...
  try {
    ...
    giriscikiskanali.send(
      `:loudspeaker: :outbox_tray: ${member.user.tag}, aramızdan ayrıldı, \**${
        sayac[member.guild.id].sayi
      }\** kişi olmamıza \**${sayac[member.guild.id].sayi -
        member.guild.memberCount}\** kişi kaldı!:x: `
    );
  } catch (e) {
    // eğer hata olursa bu hatayı öğrenmek için hatayı konsola gönderelim.
    return console.log(e);
...
client.on("guildMemberAdd", async member => {
  ...
  try {
    ...
    let giriscikiskanali = client.guilds
      ...
      .channels.get(giriscikiskanalID);
    giriscikiskanali.send(
      `:loudspeaker: :inbox_tray: ${member.user.tag}, Aramıza katıldı **${
        sayac[member.guild.id].sayi
      }** kişi olmamıza **${sayac[member.guild.id].sayi -
        member.guild.memberCount}** kişi kaldı!:ballot_box_with_check: `
    );
  } catch (e) {
    // eğer hata olursa bu hatayı öğrenmek için hatayı konsola gönderelim.
    return console.log(e);
...
client.on(`guildMemberAdd`, async member => {
  const e = new Discord.RichEmbed()
    .setColor(`RANDOM`)
    .setImage(`https://media.giphy.com/media/RNb1cm79ib28975v2F/giphy.gif`)
    .addField(
      `Sunucumuza geldiğin için teşekkür ederim!`,
      `Redno Bot sunucuda iyi eğlenceler diler.`
    )
    .addField(`Sohbete başlamadan önce;`, `Kuralları okumayı unutmayınız.`)
    .setFooter(`Bu Sunucu 7/24 Redno Bot tarafından korunuyor.`);
  member.send(e);
});
...
client.on("message", message => {
  if (message.channel.type === "dm") {
    ...
    client.channels.get("611184964930895894").send(dmlog);
  }
});